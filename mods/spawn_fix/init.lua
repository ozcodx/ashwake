...
local function is_safe_spawn_pos(pos)
    ...
    if not node or node.name == "ignore" then
        debug_print("Position not loaded: " .. minetest.pos_to_string(pos))
        return false
    end

    local node_below = minetest.get_node(vector.new(pos.x, pos.y - 1, pos.z))
    if not node_below or node_below.name == "ignore" then
        debug_print("Position below not loaded: " .. minetest.pos_to_string(vector.new(pos.x, pos.y - 1, pos.z)))
        return false
    end
    
    debug_print("Checking position: " .. minetest.pos_to_string(pos))
    debug_print("Node at position: " .. node.name)
    debug_print("Node below: " .. node_below.name)
    ...
    if node.name == "default:water_source" or 
       ...
       node_below.name == "default:lava_source" then
        debug_print("Position unsafe: in water or lava")
        return false
    end
    ...
    if node.name == "air" and not unsafe_blocks[node_below.name] then
        ...
        for y = 1, 2 do
            ...
            if not node_above or node_above.name == "ignore" or node_above.name ~= "air" then
                space_above = false
                debug_print("Not enough space above at y+" .. y .. ": " .. (node_above and node_above.name or "nil"))
                break
            end
        end
        if space_above then
            debug_print("Position is safe! Standing on: " .. node_below.name)
        end
        return space_above
    end

    debug_print("Position unsafe: not in air with suitable ground below")
    return false
end
...
local function find_safe_spawn_emerged(original_pos, callback)
    debug_print("Finding safe spawn position with emerged area from: " .. minetest.pos_to_string(original_pos))
    ...
    debug_print("Emerging area from " .. minetest.pos_to_string(min_pos) .. " to " .. minetest.pos_to_string(max_pos))
    ...
    minetest.emerge_area(min_pos, max_pos, function(blockpos, action, remaining, param)
        if remaining == 0 then
            debug_print("Area emerged, finding surface height")
            
            -- Get the surface height at the original position
            local surface_y = get_surface_height(original_pos.x, original_pos.z)
            debug_print("Surface height at (" .. original_pos.x .. "," .. original_pos.z .. ") is " .. surface_y)
            ...
            if is_safe_spawn_pos(check_pos) then
                debug_print("Found safe position at surface: " .. minetest.pos_to_string(check_pos))
                callback(check_pos)
                return
            end
            ...
            for y_offset = 1, 10 do
                ...
                if is_safe_spawn_pos(check_pos) then
                    debug_print("Found safe position above surface: " .. minetest.pos_to_string(check_pos))
                    callback(check_pos)
                    return
                end
                ...
                check_pos.y = surface_y - y_offset
                if is_safe_spawn_pos(check_pos) then
                    debug_print("Found safe position below surface: " .. minetest.pos_to_string(check_pos))
                    callback(check_pos)
                    return
                end
            end
            ...
            debug_print("No safe position at surface, trying spiral pattern...")
            local radius = 1
                ...
                for x = -radius, radius do
                    for z = -radius, radius do
                        if math.abs(x) == radius or math.abs(z) == radius then
                            ...
                            if is_safe_spawn_pos(check_pos) then
                                debug_print("Found safe position in spiral: " .. minetest.pos_to_string(check_pos))
                                callback(check_pos)
                                return
                            end
                        end
            ...
            -- If all else fails, use a default safe position
            -- This is a very simple approach: just place the player at y=10
            local fallback_pos = vector.new(original_pos.x, 10, original_pos.z)
            debug_print("No safe position found, using fallback position: " .. minetest.pos_to_string(fallback_pos))
...
local function find_safe_spawn_pos(original_pos)
    debug_print("Finding safe spawn position from: " .. minetest.pos_to_string(original_pos))
    ...
    debug_print("Trying common spawn heights...")
    for _, height in ipairs(common_heights) do
        check_pos.y = height
        if is_safe_spawn_pos(check_pos) then
            debug_print("Found safe position at common height " .. height .. ": " .. minetest.pos_to_string(check_pos))
            return check_pos
        end
    end
    
    -- If no safe position found at common heights, try a simple up/down search
    debug_print("No safe position at common heights, trying up/down search...")
    
    -- Try going up first
    for y = 1, MAX_UPWARD_CHECK do
        check_pos.y = original_pos.y + y
        if is_safe_spawn_pos(check_pos) then
            debug_print("Found safe position above: " .. minetest.pos_to_string(check_pos))
            return check_pos
        end
    end
    ...
    for y = 1, MAX_DOWNWARD_CHECK do
        check_pos.y = original_pos.y - y
        if is_safe_spawn_pos(check_pos) then
            debug_print("Found safe position below: " .. minetest.pos_to_string(check_pos))
            return check_pos
        end
    end
    
    -- If still no safe position found, try a simple spiral pattern
    debug_print("No safe position found in up/down search, trying spiral pattern...")
    local radius = 1
    ...
    debug_print("No safe position found in spiral, trying to find any position with air and solid ground...")
    for y = -50, 50 do
        check_pos.y = original_pos.y + y
        ...
        if node and node.name == "air" and node_below and node_below.name ~= "air" and
           not unsafe_blocks[node_below.name] then
            debug_print("Found position with air and suitable ground: " .. minetest.pos_to_string(check_pos))
            return check_pos
        end
    end
    
    debug_print("No safe position found, using original position")
    return original_pos
end

-- Register the spawn fix callback
debug_print("Registering spawn fix callback")
spawn.register_on_spawn(function(player, is_new)
    debug_print("Player spawn event triggered. New player: " .. tostring(is_new))
    local pos = player:get_pos()
    debug_print("Original spawn position: " .. minetest.pos_to_string(pos))
    
    -- Use emerged area approach for new players (where world might not be generated)
    if is_new then
        debug_print("New player - using emerged area approach")
        find_safe_spawn_emerged(pos, function(safe_pos)
            debug_print("Setting new player to safe position: " .. minetest.pos_to_string(safe_pos))
        ...
        debug_print("Setting player to safe position: " .. minetest.pos_to_string(safe_pos))
        player:set_pos(safe_pos)
        return true
    end
end)

debug_print("Spawn fix mod loaded successfully") 